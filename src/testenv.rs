...
impl TestEnv {
    ...
    pub fn new() -> anyhow::Result<Self> {
        ...
        // let binding = NETWORK.to_string();
        //     conf.network = &binding;

        let bitcoind = match std::env::var_os("BITCOIND_EXE") {
            // Some(bitcoind_path) => electrsd::bitcoind::BitcoinD::new(bitcoind_path),
            Some(bitcoind_path) => {
                dbg!(bitcoind_path.clone());
                electrsd::bitcoind::BitcoinD::with_conf(bitcoind_path, &conf)
            }

            None => {
                info!("download bitcoin core");
